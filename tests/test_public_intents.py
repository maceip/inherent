...
def test_slurp_loader_maps_configured_public_heads(tmp_path, monkeypatch):
    labels = [
        "lists_createoradd",
        "qa_factoid",
        "calendar_set",
...
def test_public_proxy_heads_must_be_explicitly_configured(tmp_path, monkeypatch):
    labels = ["recommendation", "calendar_query"]
    ...
    samples = intents.load_slurp(
        tmp_path / "slurp",
        public_mapping={
            "person_context": ["slurp.recommendation*"],
            "event_context": ["slurp.calendar_query"],
        },
    )

    assert len(samples) == 2
    assert samples[0].head_labels["hasPersonContext"] is True
    assert samples[1].head_labels["hasEventContext"] is True
